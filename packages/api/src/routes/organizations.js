// Organization routes - OpenAPI documentation moved to packages/api/src/openapi/
...
router.get('/:id', async (req, res) => {
    ...
    const canView = await checkPermission(req.user, 'organization:read', 'organization', id);
    if (!canView) {
...
router.put('/:id', async (req, res) => {
    ...
    const canUpdate = await checkPermission(req.user, 'organization:update', 'organization', id);
    if (!canUpdate) {
...
router.get('/:id/domains', async (req, res) => {
    ...
    // Authorization: User must have permission to update organization settings to view domains.
    const canManage = await checkPermission(req.user, 'organization:update', 'organization', id);
    if (!canManage) {
...
router.post('/:id/domains/:domainMappingId/verify', async (req, res) => {
    ...
    const canManage = await checkPermission(req.user, 'organization:update', 'organization', organizationId);
    if (!canManage) {
        return res.status(403).json({ error: 'You are not authorized to manage domains for this organization.' });
    }
