...
router.get('/:id', async (req, res) => {
    ...
    const canView = await checkPermission(req.user, ['ADMIN', 'EDITOR', 'READER'], 'project', id);
    if (!canView) {
...
router.get('/', async (req, res) => {
    try {
        const visibleProjectIds = await getVisibleResourceIdsV2(req.user, 'project');
...
router.post('/', async (req, res) => {
    ...
    const canCreate = await checkPermission(req.user, ['ADMIN', 'EDITOR'], 'team', teamId);
    if (!canCreate) {
...
router.put('/:id', async (req, res) => {
    ...
    const canUpdate = await checkPermission(req.user, ['ADMIN', 'EDITOR'], 'project', id);
    if (!canUpdate) {
...
router.delete('/:id', async (req, res) => {
    ...
    const canDelete = await checkPermission(req.user, 'ADMIN', 'project', id);
    if (!canDelete) {
...
router.post('/:id/contacts', async (req, res) => {
    ...
    const canUpdate = await checkPermission(req.user, ['ADMIN', 'EDITOR'], 'project', projectId);
    if (!canUpdate) {
...
router.get('/:id/technologies', async (req, res) => {
    ...
    const canView = await checkPermission(req.user, ['ADMIN', 'EDITOR', 'READER'], 'project', projectId);
    if (!canView) {
        return res.status(403).json({ error: 'You are not authorized to view this project.' });
    }
...
router.get('/graph', async (req, res) => {
    ...
    const isMember = await checkPermission(req.user, ['ADMIN', 'EDITOR', 'READER'], 'company', companyId);
    if (!isMember) {
...
router.get('/:id/graph', async (req, res) => {
    ...
    try {
        ...
        const isMember = await checkPermission(req.user, ['ADMIN', 'EDITOR', 'READER'], 'company', project.team.companyId);
        if (!isMember) {
            return res.status(403).json({ error: 'You are not authorized to view this project.' });
        }
...
router.post('/:projectId/link-security-tool', protect, async (req, res) => {
  ...
  try {
    const canEdit = await checkPermission(req.user, ['ADMIN', 'EDITOR'], 'project', projectId);
    if (!canEdit) {
