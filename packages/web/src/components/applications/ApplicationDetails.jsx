    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between pb-2 border-b border-white/10">
        <div>
            ...
            <p className="text-sm text-gray-400">Manage application metadata, security findings, and more.</p>
        </div>
        <div>
          {activeTab === 'Details' && (
            isEditing ? (
        ...
        {activeTab === 'Security' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => syncFindings(project.scmIntegrationId, [project.id])}
                disabled={!project.scmIntegrationId || isFindingSyncing}
                className="px-3 py-1 text-sm font-medium rounded-md bg-blue-600 hover:bg-blue-500 disabled:bg-gray-500 disabled:cursor-not-allowed"
              >
                {isFindingSyncing ? 'Syncing...' : 'Sync Findings'}
              </button>
            </div>
            <FindingList projectId={project.id} />
          </div>
        )}
